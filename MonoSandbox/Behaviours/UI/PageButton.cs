...
// Type: MonoSandbox.Behaviours.UI.PageButton
...
namespace MonoSandbox.Behaviours.UI
{
  public class PageButton : MonoBehaviour
  {
    ...
    public void Start()
    {
      this._active = this._list._currentPage == this._pageIndex;
    ...
    public void Update()
    {
      bool flag = this._list._currentPage == this._pageIndex;
    ...
    public void OnTriggerEnter(Collider other)
    {
      ...
      if (!((Component) other).TryGetComponent<GorillaTriggerColliderHandIndicator>(ref colliderHandIndicator) || colliderHandIndicator.isLeftHand || (double) Time.time <= (double) this._lastTime + 0.40000000596046448)
        return;
      ...
      if (this._list._currentPage != this._pageIndex)
      {
        this._list._currentPage = this._pageIndex;
        this._list.Clear();
        this._list.PlayAudio(false);
      }
    }
  }
}
